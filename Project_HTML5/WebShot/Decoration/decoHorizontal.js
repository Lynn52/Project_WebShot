...
// 합친 네 컷 이미지 데이터 로컬 스토리지에 저장
 function saveMerged(canvas){
    ...
    let mergedHCount = localStorage.getItem("mergedHCount");
    if(!mergedHCount){
        mergedHCount = 0;
    }else{
        mergedHCount = parseInt(mergedHCount);
    }

    //  다음 이미지 키 숫자 생성
    const nextNum = mergedHCount + 1;
    //  다음 이미지 데이터 저장
    const mergedKey = `mergedH${nextNum}`;
    const mergedData = canvas.toDataURL("image/png");
    localStorage.setItem(mergedKey, mergedData);

    //  이미지 데이터 키 숫자 업데이트
    localStorage.setItem("mergedHCount", nextNum);
}
...
btnDone.addEventListener("click", () => {
    ...
    const canvasContainer = document.querySelector(".canvas-container2");
